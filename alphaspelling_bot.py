...
#
# Notes:
#   + the token can be placed in tel_token.py to import it
#
# Requires:
#   + token: A telegram token from the @BotFather
#   + python-telegram-bot: python library (currently Version 11.1.0)
#
...
def get_mapping(word: str, lang: str) -> str:
    ...
    spelling_dict = {
        "German": german_spelling,
        "Inter": inter_spelling
    }
...
- @mitschix
    ...
    context.bot.send_message(
        chat_id=update.message.chat_id, text=start_message)
...
def wrong_conv(bot, update, user_data) -> None:
    bot.send_message(chat_id=update.message.chat_id,
                     text="sorry.. the given value is wrong!")
...
def buttons_control(update, context) -> int:
    ...
    if option == "ger":
        ...
        msg = "Thank you for the usage! If you need me again, just click /howtospell. (:"
        context.bot.send_message(chat_id=user, text=msg)
        return ConversationHandler.END
